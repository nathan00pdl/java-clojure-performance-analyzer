...
echo "[STEP 2b] Generating Java local variable table..."
if [ -f "$JAVA_CLASS" ]; then
    javap -c -p -l "$JAVA_CLASS" > "$OUTPUT_DIR/bytecode_java_locals.txt"
    echo "  ✓ bytecode_java_locals.txt"
fi
echo ""
...
echo "[STEP 3b] Generating Idiomatic Clojure local variable tables..."
{
    echo "================================================================================"
    echo "IDIOMATIC CLOJURE - LOCAL VARIABLE TABLES"
    echo "================================================================================"
    echo ""

    echo "=== MAIN FUNCTION: calculate_compound_interest ==="
    if [ -f "$IDIOMATIC_CALC" ]; then
        javap -c -p -l "$IDIOMATIC_CALC" 2>/dev/null
    fi
    echo ""

    echo "=== INNER LOOP FUNCTION (fn__275) ==="
    if [ -f "$IDIOMATIC_LOOP" ]; then
        javap -c -p -l "$IDIOMATIC_LOOP" 2>/dev/null
    fi

} > "$OUTPUT_DIR/bytecode_clojure_idiomatic_locals.txt"

echo "  ✓ bytecode_clojure_idiomatic_locals.txt"
echo ""
...
echo "[STEP 4b] Generating Clojure Java Interop local variable tables..."
{
    echo "================================================================================"
    echo "CLOJURE JAVA INTEROP - LOCAL VARIABLE TABLES"
    echo "================================================================================"
    echo ""

    echo "=== MAIN FUNCTION: calculate_compound_interest ==="
    if [ -f "$INTEROP_CALC" ]; then
        javap -c -p -l "$INTEROP_CALC" 2>/dev/null
    fi
    echo ""

    echo "=== INNER LOOP FUNCTION (fn__285) ==="
    if [ -f "$INTEROP_LOOP" ]; then
        javap -c -p -l "$INTEROP_LOOP" 2>/dev/null
    fi

} > "$OUTPUT_DIR/bytecode_clojure_interop_locals.txt"

echo "  ✓ bytecode_clojure_interop_locals.txt"
echo ""
...
echo ""
echo "  To view local variable tables:"
echo "    cat $OUTPUT_DIR/bytecode_java_locals.txt"
echo "    cat $OUTPUT_DIR/bytecode_clojure_idiomatic_locals.txt"
echo "    cat $OUTPUT_DIR/bytecode_clojure_interop_locals.txt"
echo ""