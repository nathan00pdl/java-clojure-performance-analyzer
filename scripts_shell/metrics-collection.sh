...
RESULTS_DIR="metrics-results/v3"
...
echo ""
echo "Press ENTER after configuring Gatling for $IMPLEMENTATION @ ${LOAD_LEVEL} req/s..."
echo ""
echo "Execute now: ./scripts/run-gatling.sh"
echo ""
...
GC_COUNT_FINAL=$(curl -s "${PROMETHEUS_URL}/api/v1/query?query=jvm_gc_pause_seconds_count" | jq -r '[.data.result[].value[1] | tonumber] | add // 0')
...
if [ "$GATLING_TOTAL" -gt 0 ]; then
    GATLING_ERROR_RATE=$(echo "scale=3; ($GATLING_FAILED * 100) / $GATLING_TOTAL" | bc)
else
...
if [ "$GATLING_SUCCESS" -gt 0 ]; then
    DELTA_PERCENT=$(echo "scale=3; (${DELTA#-} * 100) / $GATLING_SUCCESS" | bc)
else
    DELTA_PERCENT="0"
fi
...
cat > "$RESULT_FILE" << EOF
RESULTS - ${IMPLEMENTATION^^} @ ${LOAD_LEVEL} REQ/S
...
INTERPRETATION (thresholds calibrated for ${LOAD_LEVEL} req/s)
────────────────────────────────────────────────
EOF
...
if (( $(echo "$PERCENTILE_95 < $P95_EXCELLENT" | bc -l) )); then
    echo "P95 Latency: ✓ EXCELLENT (< ${P95_EXCELLENT}ms at ${LOAD_LEVEL} req/s)" >> "$RESULT_FILE"
elif (( $(echo "$PERCENTILE_95 < $P95_GOOD" | bc -l) )); then
    echo "P95 Latency: ✓ GOOD (< ${P95_GOOD}ms at ${LOAD_LEVEL} req/s)" >> "$RESULT_FILE"
elif (( $(echo "$PERCENTILE_95 < $P95_ACCEPTABLE" | bc -l) )); then
    echo "P95 Latency: ⚠ ACCEPTABLE (< ${P95_ACCEPTABLE}ms at ${LOAD_LEVEL} req/s)" >> "$RESULT_FILE"
else
    echo "P95 Latency: ✗ HIGH (> ${P95_ACCEPTABLE}ms at ${LOAD_LEVEL} req/s)" >> "$RESULT_FILE"
fi

if (( $(echo "$PERCENTILE_99 < $P99_EXCELLENT" | bc -l) )); then
    echo "P99 Latency: ✓ EXCELLENT (< ${P99_EXCELLENT}ms at ${LOAD_LEVEL} req/s)" >> "$RESULT_FILE"
elif (( $(echo "$PERCENTILE_99 < $P99_GOOD" | bc -l) )); then
    echo "P99 Latency: ✓ GOOD (< ${P99_GOOD}ms at ${LOAD_LEVEL} req/s)" >> "$RESULT_FILE"
elif (( $(echo "$PERCENTILE_99 < $P99_ACCEPTABLE" | bc -l) )); then
    echo "P99 Latency: ⚠ ACCEPTABLE (< ${P99_ACCEPTABLE}ms at ${LOAD_LEVEL} req/s)" >> "$RESULT_FILE"
else
    echo "P99 Latency: ✗ HIGH (> ${P99_ACCEPTABLE}ms at ${LOAD_LEVEL} req/s)" >> "$RESULT_FILE"
fi
...
if (( $(echo "$GC_COUNT == 0" | bc -l) )); then
    echo "GC Activity: ✓ PERFECT (No GC events)" >> "$RESULT_FILE"
elif (( $(echo "$GC_COUNT < 5" | bc -l) )); then
    echo "GC Activity: ✓ EXCELLENT (< 5 collections)" >> "$RESULT_FILE"
elif (( $(echo "$GC_COUNT < 15" | bc -l) )); then
    echo "GC Activity: ✓ GOOD (5-15 collections)" >> "$RESULT_FILE"
elif (( $(echo "$GC_COUNT < 30" | bc -l) )); then
    echo "GC Activity: ⚠ MODERATE (15-30 collections)" >> "$RESULT_FILE"
else
...
echo "    Load Level:                ${LOAD_LEVEL} req/s"
echo ""
...
echo "  2. Run next test with appropriate parameters:"
echo -e "     ${GREEN}./scripts/metrics-collection.sh [java|clojure-idiomatic|clojure-interop-java] [100|500|1000]${NC}"
echo ""